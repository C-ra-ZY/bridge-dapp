...
const KEY_WALLET_STORAGE = "__plug_wallet_storage";
interface PlugState {
...
class PlugKeyRing {
  ...
  public init = async (): Promise<void> => {
    const state = JSON.parse(localStorage.getItem(KEY_WALLET_STORAGE) || "{}");
  ...
  private loadFromPersistance = async (password: string): Promise<void> => {
    ...
    const state = JSON.parse(localStorage.getItem(KEY_WALLET_STORAGE) || "{}");
    const { vault, isInitialized } = (state || {}) as StorageData;
  ...
  public unlock = async (password: string): Promise<boolean> => {
    ...
    try {
      ...
      let storageState = JSON.parse(
        localStorage.getItem(KEY_WALLET_STORAGE) || "{}"
      );
      storageState = Object.assign(storageState, {
        isUnlocked: this.isUnlocked,
      });
      localStorage.setItem(KEY_WALLET_STORAGE, JSON.stringify(storageState));
  ...
  private saveEncryptedState = async (newState, password): Promise<void> => {
    ...
    storageState = Object.assign(storageState, { vault: encrypted });
    localStorage.setItem(KEY_WALLET_STORAGE, JSON.stringify(storageState));
  };
